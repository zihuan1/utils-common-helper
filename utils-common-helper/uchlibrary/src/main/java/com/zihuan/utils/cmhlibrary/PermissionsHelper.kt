...
inline fun storage(crossinline action: (Boolean) -> Unit) {
    storagePermission { action(it) }
}

inline fun storageTrue(crossinline action: () -> Unit) {
    storagePermission { if (it) action() }
}
inline fun storageFalse(crossinline action: () -> Unit) {
    storagePermission { if (!it) action() }
}
...
inline fun requestEasyPermission(
    vararg permissions: String, crossinline action: (pass: Boolean) -> Unit
) {
